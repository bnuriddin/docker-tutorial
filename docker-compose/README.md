...
```
cd docker_tutorial/docker-compose
```
...
3. Смотрим на содержание папки `docker-tutorial`. В ней нужно подправить два файла:
...
server {
    listen       80;
    server_name  localhost; <-- подправить!
```
...
```
...
curl -H "Host: localhost" --silent --show-error --fail -I http://nginx/ <-- подправить!
...
curl -H "Host: localhost" --silent --show-error --fail -I http://nginx/phpinfo/ <-- подправить!
```
...
7. Подключимся к нашему сайту из браузера, используя режим Инкогнито, на адреса `http://localhost` и `localhost/phpinfo/`
...
Перейти в каталог `docker_tutorial/work_yourself`.
